/**
 * Vendored from paseo v0.9.2 `packages/plugin/src/server/acp-internal/connection.ts`
 * (Apache-2.0, © Mohamed Boudra), plus local changes to support `prompt.steer`:
 *
 *   - `ADAPTER_CAPABILITIES` advertises `prompt.steer`;
 *   - `admitPrompt` steers instead of interrupting: a `delivery: "steer"`
 *     prompt keeps the running turn's identity (the bridge preempts the old
 *     backend turn when the new session/prompt lands mid-flight), reports
 *     `prompt_result { type: "steer" }`, and a generation counter silences the
 *     superseded request's terminal event so the paseo turn never reads as
 *     cancelled;
 *   - `clearPendingPermissions` cancels outstanding permission requests
 *     before steering (the daemon asks for this so a parked popup cannot
 *     strand the steer).
 *
 * The stock adapter refuses steer prompts outright (never advertises the
 * capability); everything else matches upstream except import paths. Re-sync
 * when upgrading the pinned `@getpaseo/plugin` dependency.
 */
...
// Local change vs upstream: `prompt.steer` is advertised and honored — see
// admitPrompt's steer branch below.
...
class AcpRuntime {
  ...
  /** Local change vs upstream: bumped per dispatched prompt; see beginPrompt. */
  ...
  // Local change vs upstream: the parameter property is expanded into a
  // declared field + assignment — Node's type-stripping (erasable-syntax
  // only) rejects constructor parameter properties.
  private constructor(
    stream: Stream,
  ...
  private async admitPrompt(
    ...
    if (input.prompt.delivery === "steer") {
      // Local change vs upstream: steer into the running turn instead of
      // refusing the prompt. With no live turn there is nothing to steer
      // into — report completion with zero side effects so the daemon's
      // steer-admission fallback re-dispatches the message as a normal turn.
      if (!active) {
        this.emit({
          type: "session.prompt_result",
          sessionId: this.options.boundarySessionId,
      ...
      if (input.prompt.clearPendingPermissions) this.cancelPendingPermissions();
      // The bridge (zcode-acp-server) preempts the superseded backend turn
      // when this prompt lands mid-flight, so no explicit cancel is sent
      // first — the generation guard in beginPrompt silences the superseded
      // request's terminal event below.
      const turnId = active.turnId;
      // prompt_result first: the daemon's recordAcceptedSteer runs on it and
      // registers the steered message for this clientMessageId. No user_message
      // echo and no turn started — the turn keeps its identity and the daemon
      // already renders the steered message (reconcileSubmittedPromptEcho
      // would only run before that record exists).
      this.emit({
        type: "session.prompt_result",
        sessionId: this.options.boundarySessionId,
        clientMessageId: input.prompt.clientMessageId,
      ...
      this.beginPrompt(toAcpPrompt(input.prompt), turnId);
      return;
    }
    if (active) {
    ...
    this.beginPrompt(prompt, turnId);
  }

  /**
   * Dispatch an ACP session/prompt and own its terminal translation. Local
   * change vs upstream: a generation counter (bumped per dispatch) silences
   * the terminal event of a request superseded by a steer — the preempted
   * request settles "cancelled" and must not cancel the still-running paseo
   * turn, and repeated steers chain the same way. Normal (non-steer)
   * dispatches always run with the current generation because admitPrompt
   * waits for the active request to settle first, so upstream behavior is
   * unchanged on that path.
   */
  private beginPrompt(prompt: ContentBlock[], turnId: string): void {
    ...
    const settled = this.call(
      ...
      (response): void => {
        ...
        this.finishTurn(turnId, state);
      },
      (error): void => {
        if (this.promptGeneration !== generation) return;
        this.finishTurn(turnId, "failed", { message: describeError(error) });
      },
    );
    const entry = { turnId, settled };
    this.activePrompt = entry;
    void settled.finally(() => {
      if (this.activePrompt === entry) this.activePrompt = null;
    });
  }
  ...
  /** Local change vs upstream: deny outstanding permissions before a steer. */
